...
func TestChunkedPrefill(t *testing.T) {
	...
	RunSpecs(t, "Chunked Prefill Suite (all-GPU TP)")
}
...
var _ = Describe("Chunked Prefill", Label("pass-fail"), func() {

	Describe("vLLM Server", func() {
		...
		It("should have model loaded", func() {
			checkModels(serverURL)
		})
		It("should serve inference requests", func() {
			runInference(serverURL, modelName)
		})
	})

})

var _ = Describe("Chunked Prefill Stress Test", Label("quantitative"), func() {

	Describe("Server Health", func() {
		It("should be reachable", func() {
			Expect(serverURL).NotTo(BeEmpty(), "SERVER_URL must be set")
			healthCheck(serverURL)
		})
	})
	...
	Describe("Stress Test", func() {
		...
		It("should report stress test results", func() {
			...
			GinkgoWriter.Printf("=== Stress Test Results ===\n")
